...
Every agent is a while loop. The model reads the message history, decides what to do, calls a tool or stops. Repeat. Thirty lines of Python.
...
The agent isn't the interesting part. Every framework wraps the same thirty lines. What matters is the exit condition: how you validate the output, and whether your task actually needs an agent at all. The framework gives you the scaffolding. You still have to get the stopping right.

All experiments ran on **Gemini 3.1 Flash Lite** through LangGraph. A different model will produce different loop counts, different hallucination rates, and different exit behavior. The failure modes stay the same.
