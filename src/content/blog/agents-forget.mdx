...
An agent's memory is its message history. Every turn appends to it, and every turn the model re-reads all of it. By turn 10 you're paying to re-send turn 1 ten times. By turn 20 the context is large enough that something has to be cut, summarized, or moved out of the conversation entirely. The four strategies below handle that differently, and the one with perfect recall isn't the one you'd pick for a real system.

{/* IMAGE: hero diagram showing the 4 strategies side by side as mental models — baseline (messages dropping off the front), sliding window (fixed-size window), summary (messages collapsing to a bullet), scratchpad (messages flowing into an external box) */}
...
**Baseline** drops the oldest messages when the context exceeds the budget. No intelligence, no preservation. The default if you don't write a context manager at all.
...
|----------|----------|-----------|--------|------|--------|-----|--------------|
| Baseline | 0.50 | 0.00 | 25,780 | $0.003 | 0.67 | 0.14 | 0.00 |
...
|----------|----------------|--------------|
| Baseline | Old facts | Forgets gradually as the budget fills |
| Sliding Window | Old facts | Drops in chunks, hits a cliff |
| Running Summary | Precision | Facts blur with each compression pass |
| Scratchpad | Message budget | System message grows until no room is left for the conversation |
